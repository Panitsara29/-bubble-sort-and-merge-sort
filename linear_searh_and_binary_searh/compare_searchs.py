...
def on_draw():
    ...
    for i, number in enumerate(numbers):
        ...
        color_linear = (98, 206, 241) if linear_found and i == linear_index - 1 else (183, 167, 255) if i == linear_index else (200, 200, 200)
        ...
        color_binary = (98, 206, 241) if binary_found and i == binary_mid else (183, 167, 255) if binary_left <= i <= binary_right else (200, 200, 200)
        ...
        label = pyglet.text.Label(str(number), x=x + box_size/2, y=y_linear + box_size/2, anchor_x='center', anchor_y='center', color=(50, 50, 50, 255), batch=batch)
        label.draw()
        label = pyglet.text.Label(str(number), x=x + box_size/2, y=y_binary + box_size/2, anchor_x='center', anchor_y='center', color=(50, 50, 50, 255), batch=batch)
        label.draw()
